...
export function loadSchedule(env = process.env) {
  return loadScheduleWithMeta(env).windows;
}

export const SCHEDULE_CACHE_VERSION = 1;

/**
 * @typedef {object} LoadedEnvSchedule
 * @property {Array} windows normalized windows
 * @property {"env"} source
 *
 * @typedef {object} LoadedCacheSchedule
 * @property {Array} windows normalized windows
 * @property {"cache"} source
 * @property {{fetchedAt?: string, sourceUrl?: string, excerpt?: string}} meta
 * @property {string} cachePath
 *
 * @typedef {object} LoadedBuiltinSchedule
 * @property {Array} windows normalized windows
 * @property {"builtin"} source
 * @property {string} cachePath
 * @property {string} [cacheIssue] set when a cache file exists but is invalid
 */

export function defaultCachePath(env = process.env) {
  const xdg = env?.XDG_CACHE_HOME;
  if (xdg != null && String(xdg).trim() !== "") {
    return `${String(xdg).trim().replace(/\/+$/, "")}/deepseek-peak/schedule.json`;
  }
  return `${homedir()}/.cache/deepseek-peak/schedule.json`;
}

export function resolveCachePath(env = process.env) {
  const override = env?.DEEPSEEK_PEAK_CACHE;
  if (override != null && String(override).trim() !== "") return String(override).trim();
  return defaultCachePath(env);
}

/**
 * Read a cache file written by `deepseek-peak sync`. Never throws.
 * @returns {{status:"missing"}} | {{status:"invalid", error:string}} |
 *          {{status:"ok", windows, meta:{{fetchedAt?, sourceUrl?, excerpt?}}}}
 */
export function readCacheFile(cachePath) {
  let text;
  try {
    text = readFileSync(cachePath, "utf8");
  } catch (err) {
    if (err.code === "ENOENT") return { status: "missing" };
    return { status: "invalid", error: `unreadable (${err.code})` };
  }
  let data;
  try {
  ...
  if (!data || typeof data !== "object") return { status: "invalid", error: "not an object" };
  if (data.version !== SCHEDULE_CACHE_VERSION) {
    return { status: "invalid", error: `unsupported version (want ${SCHEDULE_CACHE_VERSION})` };
  }
  try {
    const windows = normalizeWindows(data.schedule);
    return {
      status: "ok",
      windows,
      meta: { fetchedAt: data.fetchedAt, sourceUrl: data.sourceUrl, excerpt: data.excerpt },
    };
  } catch (err) {
    return { status: "invalid", error: `bad schedule: ${err.message}` };
  }
}

/** Days since the cache was fetched (Infinity when unknown). */
export function cacheAgeDays(meta, now = new Date()) {
  if (!meta || typeof meta.fetchedAt !== "string") return Infinity;
  const ageMs = now.getTime() - Date.parse(meta.fetchedAt);
  if (Number.isNaN(ageMs)) return Infinity;
  return ageMs / 86400000;
}

/**
 * Load the schedule plus where it came from. Precedence: ENV override >
 * sync cache file > built-in defaults.
 * @returns {LoadedEnvSchedule | LoadedCacheSchedule | LoadedBuiltinSchedule}
 */
export function loadScheduleWithMeta(env = process.env) {
  ...
  if (raw != null && String(raw).trim() !== "") {
    ...
    return { windows: normalizeWindows(parsed), source: "env" };
  }
  const cachePath = resolveCachePath(env);
  const cached = readCacheFile(cachePath);
  if (cached.status === "ok") return { windows: cached.windows, source: "cache", meta: cached.meta, cachePath };
  if (cached.status === "invalid") {
    return { windows: normalizeWindows(DEFAULT_WINDOWS), source: "builtin", cachePath, cacheIssue: cached.error };
  }
  return { windows: normalizeWindows(DEFAULT_WINDOWS), source: "builtin", cachePath };
}
...
/**
 * The nearest schedule transition strictly after `date` — i.e. the nearest
 * boundary at which the peak state actually *changes*. Inner boundaries of
 * overlapping/adjacent windows (peak before and after) are skipped, so
 * callers never see phantom transitions.
 * @returns {{ at: Date, to: "peak" | "offpeak", inMs: number }}
 */
export function nextTransition(date = new Date(), windows = loadSchedule()) {
  const norm = isNormalized(windows) ? windows : normalizeWindows(windows);
  const nowMs = date.getTime();
  const peakNow = isPeak(date, norm);
  ...
  for (let dayOffset = 0; dayOffset <= 8; dayOffset++) {
    ...
    for (const w of norm) {
      ...
      if (startMs > nowMs) candidates.add(startMs);
      if (endMs > nowMs) candidates.add(endMs);
    }
    ...
    for (const w of norm) {
      ...
      if (tailEndMs > nowMs) candidates.add(tailEndMs);
    }
  }
  if (candidates.size === 0) throw new Error("No upcoming transition found (empty schedule?)");
  const sorted = [...candidates].sort((a, b) => a - b);
  // State only changes at boundaries, so the first boundary with a flipped
  // state is the next real transition. (A schedule that is peak 24/7 has
  // none — that misconfiguration fails loud here instead of looping.)
  const atMs = sorted.find((ms) => isPeak(new Date(ms + 1000), norm) !== peakNow);
  if (atMs === undefined) throw new Error("No upcoming transition found (peak 24/7?)");
